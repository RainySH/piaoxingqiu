...
class Piaoxingqiu:
    ...
    def initialize_audience(self, token, audiences) -> None:
        ...
        for audience_to_delete in audiences_to_delete:
            url_to_delete = url + '/' + audience_to_delete['id'] + '?id=%s' % (audience_to_delete)
            response = requests.delete(url=url_to_delete, headers=headers).json()
        
        if not self.get_audiences(token):
            print("删除观影人成功！")
        else:
            raise Exception("删除观影人失败：" + str(response))

        # 添加观影人
        for audience in audiences:
            response = requests.post(url=url, headers=headers, json=audience).json()
            if response["statusCode"] == 200:
                print("添加观影人（%s）成功！" % (audience['name']))
            else:
                raise Exception("删除观影人（%s）失败：" % (audience['name']) + str(response))
    ...
    def get_show(self, show_id) -> dict | None:
        ...
        url = "https://m.piaoxingqiu.com/cyy_gatewayapi/show/pub/v3/show/" + show_id + "/sessions_from_marketing_countdown"
        ...
        show = {
            ...
            'sessions' : {session['bizShowSessionId'] : {'session_id' : session['bizShowSessionId'], 'session_date' : session['sessionName']} for session in response['data']['sessionVOs']}
        }
        ...
        for session_id in show['sessions']:
            ...
            response = requests.get(url=url, headers=headers).json()
            show['sessions'][session_id]['seat_plans'] = {seat_plan['seatPlanId'] : {'seat_plan_id' : seat_plan['seatPlanId'], 'seat_plan_price' :seat_plan['originalPrice'], 'seat_plan_name' : seat_plan['seatPlanName']} for seat_plan in response['data']['seatPlans']}

        if response["statusCode"] != 200:
            raise Exception("get_shows异常:" + str(response))
    ...
    def get_address(self, token) -> dict | None:
        """获取收货地址
        """
        headers = {
            'User-Agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Mobile Safari/537.36',
    ...
    def get_express_fee(self, token, show_id, session_id, seat_plan_id, price: int, qty: int, location_city_id: str) -> dict:
        """获取快递费
        """
        headers = {
            'User-Agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Mobile Safari/537.36',
    ...
    def create_order(self, token, show_id, session_id, seat_plan_id, price: int, qty: int, deliver_method, express_fee: int, receiver,
                    cellphone,
                    address_id, detail_address, location_city_id, audience_ids: list) -> bool:
        ...
        elif deliver_method in ["E_TICKET", "ID_CARD", "VENUE", "VENUE_E"]:
            data = {
                "priceItemParam": [
                    {
                        "applyTickets": [],
